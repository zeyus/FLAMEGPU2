...
"""
    ...
    def test_DuplicateOutputException(self): 
        ...
        # Shuffle the list of numbers
        const unsigned seed = static_castUInt(std::chrono::system_clock::now().time_since_epoch().count())
    ...
    def test_UnsetLength(self): 
        m = pyflamegpu.ModelDescriptionodel(MODEL_NAME)
    ...
    def test_ReadEmpty(self): 
        ...
        m = pyflamegpu.ModelDescriptionodel("Model")
           # Location message
           ...
           # Circle agent
            ...
            agent.newRTCFunction("in", countArray).setMessageInput("location")
        ...
        c = pyflamegpu.CUDAAgentModeluda_model(model)
        ...
        assert pop_out.getCurrentListSize() == 1u
        auto ai = pop_out.getInstanceAt(0)
        assert ai.getVariableUInt("value") == 0u  # Unset array msgs should be 0
