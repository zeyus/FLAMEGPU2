...
class DeviceAPI {
    // Friends have access to TID() & TS_ID()
    template<typename AgentFunction, typename _MessageIn>
...
template<typename MessageIn>
template<typename T, unsigned int N>
__device__ T DeviceAPI<MessageIn>::getVariable(const char(&variable_name)[N]) const {
    // simple indexing assumes index is the thread number (this may change later)
    const unsigned int index = (blockDim.x * blockIdx.x) + threadIdx.x;
...
template<typename MessageIn>
template<typename T, unsigned int N, unsigned int M>
__device__ T DeviceAPI<MessageIn>::getVariable(const char(&variable_name)[M], const unsigned int &array_index) const {
    // simple indexing assumes index is the thread number (this may change later)
    const unsigned int index = (blockDim.x * blockIdx.x) + threadIdx.x;
...
template<typename MessageIn>
template<typename T, unsigned int N>
__device__ void DeviceAPI<MessageIn>::setVariable(const char(&variable_name)[N], T value) {
...
template<typename MessageIn>
template<typename T, unsigned int N, unsigned int M>
__device__ void DeviceAPI<MessageIn>::setVariable(const char(&variable_name)[M], const unsigned int &array_index, const T &value) {
    if (variable_name[0] == '_') {
#if !defined(SEATBELTS) || SEATBELTS
        DTHROW("Variable names starting with '_' are reserved for internal use, with '%s', in DeviceAPI::setVariable().\n", variable_name);
#endif
