...
* Sets the array index to store the message in
*/
__device__ inline void MsgArray2D::Out::setIndex(const size_type &x, const size_type &y) const {
        unsigned int index = (blockDim.x * blockIdx.x) + threadIdx.x;
        size_type index_1d =
            y * metadata->dimensions[0] +
            x;
...
__device__ inline MsgArray2D::In::Filter::Filter(const MetaData *_metadata, const Curve::NamespaceHash &_combined_hash, const size_type &x, const size_type &y, const size_type &_radius)
    : radius(_radius)
    , metadata(_metadata)
...
__device__ inline MsgArray2D::In::Filter::Message& MsgArray2D::In::Filter::Message::operator++() {
